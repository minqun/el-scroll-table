<template>
  <div
    class="el-table el-scrollbar el-scrollbar-table"
    ref="etable"
    v-if="!$isServer"
    :class="[
      {
        ...
        'el-table--scrollable-x': layout != null ? layout.scrollX : false,
        'el-table--scrollable-y': layout != null ? layout.scrollY : false,
        'el-table--enable-row-hover': store ? !store.states.isComplex : false,
        'el-table--enable-row-transition': store
  ...
  >
    ...
    <div
      class="el-table__body-wrapper el-scrollbar__wrap"
      ref="bodyWrapper"
      :class="[
    ...
    >
      <div
        ...
        @mousedown="clickThumbHandlerTable"
        ref="resize"
      >
        <table-body
          :context="context"
    ...
    <Bar ref="horizontal" :move="moveX" :size="sizeWidth"></Bar>
    <Bar ref="vertical" :vertical="true" :move="moveY" :size="sizeHeight"></Bar>
    <div
      v-if="showSummary && !isHeadSummer"
      v-show="data && data.length > 0"
      v-mousewheel="handleHeaderFooterMousewheel"
      @mousedown="clickThumbHandlerTable"
      class="el-table__footer-wrapper"
    ...
    >
      ...
      <div
        v-if="showSummary"
        :style="`${isHeadSummer ? 'bottom: inherit;top:38px;z-index:4;' : ''}`"
        v-show="data && data.length > 0"
      ...
      >
        <div
          ...
          :style="
            `${
              isHeadSummer
                ? 'bottom: inherit;position:relative;top: -0.5px'
                : ''
            }`
          "
          v-show="data && data.length > 0"
...
export default {
  ...
  methods: {
    handleScroll() {
      ...
      this.moveY = (wrap.scrollTop * 100) / (wrap.clientHeight + this.fineSize);
      this.moveX = (wrap.scrollLeft * 100) / wrap.clientWidth;
    },
    // 表触
    clickThumbHandlerTable(e) {
      if (e.ctrlKey || e.button == 2) {
        return;
      }
      if (this.overall) {
        this.startDrag(e);
        this[this.bar.axis] = e.x;
      }
    },
    // clickThumbHandler(e) {
    //   if (e.ctrlKey || e.button == 2) {
    ...
    mouseMoveDocumentHandler(e) {
      ...
      if (!prePage) return;
      const offset = this.$el.getBoundingClientRect()[this.bar.direction];
      const ref = this.$children.filter(ev => !ev.vertical)[0].$el;
      let z = e.x - this[this.bar.axis];
      let pz = 0.01 * (1 - this.sizeWidth.replace("%", "")) * 0.5 * z;
      this.wrap[this.bar.scroll] = pz + this.bodyWrapper.scrollLeft;
    },
    ...
    update() {
      ...
      widthPentage = (wrap.clientWidth * 100) / wrap.scrollWidth;
    ...
    syncPostion: throttle(20, function() {
      ...
      // console.log('scrollLeft, scrollTop, offsetWidth, scrollWidth', scrollLeft, scrollTop, offsetWidth, scrollWidth, this.bodyWrapper);
      const {
        headerWrapper,
        footerWrapper,
        fixedBodyWrapper,
    ...
    doLayout() {
      ...
      if (
        this.$refs.etable &&
        this.$refs.etable.querySelectorAll("td .cell").length
      ) {
        this.bindDbl = true;
        this.$refs.etable.querySelectorAll("td .cell").forEach(item => {
          item.ondblclick = e => {
  ...
  data() {
    let layout = null;
    console.log("this.$isServer", this,this.$isServer);
    if (!this.$isServer) {
      const {
        hasChildren = "hasChildren",
        children = "children"
      ...
      if (gutter) {
        ...
        const gutterStyle = `margin-bottom: ${gutterWidth};`;
        const _this = this;
        // 可配置化设置
